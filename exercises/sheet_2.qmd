...
Solutions: [Sheet 2 Solutions](sheet_2_solutions.qmd)

<a class="lecture-download-link" href="original_files/exercise02.pdf?download=1" download>Download original exercise PDF</a>
...
ax.set_title("Plot of arctan(x) versus x")
plt.show()
```
...
Hint: when you need multiple independent plots, create a new figure for each using `fig, ax = plt.subplots()` and then call `ax.plot(...)`, `ax.set_xlabel(...)`, etc.
...
ax1.set_xlabel("t")
ax1.set_ylabel("u(t)")
ax1.set_title("Approximate solution u_n")
plt.show()
...
ax2.plot(td, errors)
ax2.set_xlabel("t")
ax2.set_ylabel("|e^{a t} - u_n|")
ax2.set_title("Error between exact and approximate solution")
plt.show()
```
...
fig, ax = plt.subplots()
ax.plot(td, exact, label="Exact: exp(a t)")
ax.plot(td, u, label="Approximation u_n")
ax.set_xlabel("t")
ax.set_ylabel("Value")
ax.set_title("Exact vs Approximate Solution")
ax.legend()
plt.show()
```
...
<nav class="bottom-page-nav" aria-label="Page navigation">
  <span class="page-nav-link prev is-disabled">Previous</span>
  <a class="page-nav-link next" href="sheet_3.html">Next: Sheet 3</a>
</nav>
