...
def poly_general(x, coeffs):
    ...
    https://en.wikipedia.org/wiki/Horner_method"""
    result = 0.0
    for coeff in coeffs:
...
fig, ax = plt.subplots()
ax.plot(omegas, values)
ax.set_xlabel("omega")
ax.set_ylabel("Integral value")
ax.set_title("Integral of sin(omega x) from 0 to pi/2")
plt.show()
```
...
fig, ax = plt.subplots()
ax.plot(a_vals, roots, marker="o")
ax.set_xlabel("a")
ax.set_ylabel("Positive root")
ax.set_title("Positive roots of a x^2 + x - 3")
plt.show()
```

<nav class="bottom-page-nav" aria-label="Page navigation">
  <a class="page-nav-link prev" href="sheet_4_solutions.html">Previous: Sheet 4 Solutions</a>
  <a class="page-nav-link next" href="sheet_6_solutions.html">Next: Sheet 6 Solutions</a>
</nav>
