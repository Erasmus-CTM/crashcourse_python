...
<a class="lecture-download-link" href="original_files/lecture05.ipynb?download=1" download>Download original notebook (.ipynb)</a>
...
```{pyodide-python}
def bisec1(f, _interval, tol):
    interval = _interval.copy()
    for i in range(100):
...
print("mid = ", m)
# error, interval has changed because lists are mutable as function parameters 
make_plot(f, interval, m)  # the plot must be made before bisec1 is called
```

Why does the above code not? **(5min, let's go!)**
...
<nav class="bottom-page-nav" aria-label="Page navigation">
  <a class="page-nav-link prev" href="lecture04.html">Previous: Lecture 4</a>
  <a class="page-nav-link next" href="lecture06.html">Next: Lecture 6</a>
</nav>
